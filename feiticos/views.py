...
from .mercado_pago_utils import (
    ...
    extrair_dados_webhook,
    mapear_status_mp_para_agendamento
)
...
def agendar_produto(request):
    ...
    try:
        ...
        try:
            ...
            print(f"ERRO DE BANCO DE DADOS: {str(db_err)}")
            messages.error(request, f"Erro ao salvar no banco de dados. Por favor, tente novamente. Detalhes: {str(db_err)}")
            return redirect('index')
...
def pagamento(request, pk):
    """Página de pagamento"""
    try:
        # No MongoDB/Djongo, buscar por PK de forma resiliente
        ...
        messages.error(request, f'Erro ao carregar pagamento: {str(e)}')
        return redirect('index')


def confirmar_pagamento(request, pk):
    """Cria preferência no Mercado Pago e redireciona para checkout"""
    try:
        agendamento = Agendamento.objects.filter(pk=pk).first()
        if not agendamento:
            messages.error(request, 'Agendamento não encontrado.')
        ...
        preferencia = criar_preferencia_mercado_pago(agendamento)
...
def agendar_feitico(request, token):
    ...
    try:
        ...
        if not all([tipo_feitico, data_agendamento]):
            messages.error(request, 'Por favor, preencha todos os campos.')
            return redirect('comprar_feitico', token=token)
...
def obter_datas_disponiveis(request):
    """API para obter datas disponíveis - Otimizada para MongoDB Atlas"""
    tipo_produto = request.GET.get('tipo_produto', 'tarot')
    datas_disponiveis = []
    
    # 1. Carregar indisponibilidades uma única vez (evita SQLDecodeError em loop)
    # Buscamos todas e filtramos via Python para garantir estabilidade
    todas_indisp = list(IndisponibilidadeFeitico.objects.all())
    datas_indisp = {str(i.data) for i in todas_indisp if i.indisponivel}
    
    # 2. Carregar agendamentos do período para contar vagas via Python
    hoje = datetime.now().date()
    fim_periodo = hoje + timedelta(days=60)
    
    # Filtro simples que o Djongo suporta melhor
    status_validos = ['pagamento_confirmado', 'pago', 'enviar_link', 'link_enviado', 'realizado']
    agendamentos_periodo = list(Agendamento.objects.filter(
        data_agendamento__gte=hoje,
        data_agendamento__lte=fim_periodo
    ))
    ...
    for i in range(60):
        ...
        # 1. Verificar indisponibilidade (Python set)
        if data_str in datas_indisp:
            continue
            
        # 2. Filtrar agendamentos do dia em memória
        ag_no_dia = [ag for ag in agendamentos_periodo if ag.data_agendamento == data]
        
        # 3. Lógica de vagas (replicando regras de negócio via Python)
        dias_uteis = [0, 1, 2, 3, 4] # Segunda a Sexta
        vagas_ok = False
        
        if tipo_produto == 'tarot_acompanhamento':
            if data.weekday() == 4: # Apenas Sexta
                usadas = len([ag for ag in ag_no_dia if ag.tipo_produto == 'tarot_acompanhamento' and ag.status in status_validos])
                if usadas < 10: # VAGAS_ACOMPANHAMENTO
                    vagas_ok = True
        elif tipo_produto == 'feitico':
            if data.weekday() in dias_uteis:
                usadas = len([ag for ag in ag_no_dia if ag.tipo_produto == 'feitico' and ag.status in status_validos])
                if usadas < 3: # VAGAS_FEITICO
                    vagas_ok = True
        elif tipo_produto in ['tarot', 'intencao_feitico']:
            if data.weekday() in dias_uteis:
                usadas_tarot = len([ag for ag in ag_no_dia if ag.tipo_produto == 'tarot' and ag.status in status_validos])
                usadas_intencao = len([ag for ag in ag_no_dia if ag.tipo_produto == 'intencao_feitico' and ag.status in status_validos])
                if (usadas_tarot + usadas_intencao) < 10: # VAGAS_TAROT
                    vagas_ok = True
                    
        if vagas_ok:
            datas_disponiveis.append(data_str)
...
def admin_editar_agendamento(request, pk):
    """Página para editar um agendamento específico"""
    try:
        agendamento = Agendamento.objects.filter(pk=pk).first()
        if not agendamento:
            messages.error(request, 'Agendamento não encontrado.')
...
def admin_gerenciar_produtos(request):
    ...
    feiticos = list(TipoFeitico.objects.all())
    tarots = list(TipoTarot.objects.all())
    
    # Converter Decimal128 para float para evitar sumiço no template
    for f in feiticos:
        try:
            f.preco = float(f.preco)
        except (TypeError, ValueError, AttributeError):
            if hasattr(f.preco, 'to_decimal'):
                f.preco = float(f.preco.to_decimal())
                
    for t in tarots:
        try:
            t.preco = float(t.preco)
        except (TypeError, ValueError, AttributeError):
            if hasattr(t.preco, 'to_decimal'):
                t.preco = float(t.preco.to_decimal())
    ...
    if request.method == 'POST':
        ...
        if acao == 'adicionar':
            ...
            try:
                if tipo == 'feitico':
                    # Busca resiliente para MongoDB
                    produto = TipoFeitico.objects.filter(pk=produto_id).first()
                    if not produto:
                    ...
                    # Busca resiliente para MongoDB
                    produto = TipoTarot.objects.filter(pk=produto_id).first()
                    if not produto:
                        messages.error(request, 'Tarot não encontrado.')
                        return redirect('admin_gerenciar_produtos')
                ...
                messages.error(request, f'Erro ao editar: {str(e)}')
